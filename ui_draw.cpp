...
void initSprites() {
    sprHeader.createSprite(240, 36);   
    sprSensors.createSprite(240, 140); 
    sprRelays.createSprite(240, 44);   
    sprFooter.createSprite(240, 20);   
}
...
void updateHeader(bool wifiOn, bool serverOn) {
    if (xSemaphoreTake(uiMutex, portMAX_DELAY)) {
        ...
        for (int i = 0; i < 36; i++) {
            uint16_t color = sprHeader.color565(0, i, i*1.5); 
        ...
        sprHeader.drawString("OrchidPal", 10, 18);
...
void updateSensors(float temp, float humid, int soil, int light) {
    if (xSemaphoreTake(uiMutex, portMAX_DELAY)) {
        ...
        int w = (240 - 12) / 2;
        drawOneCard(4, 4, w, 64, "TEMP", temp, "C", temperature_icon, COL_TEMP, hasTemp);
        drawOneCard(4 + w + 4, 4, w, 64, "HUMID", humid, "%", humidity_icon, COL_HUMID, hasHumid);
        drawOneCard(4, 72, w, 64, "SOIL", soil, "%", leaf_icon, COL_SOIL, hasSoil);
        drawOneCard(4 + w + 4, 72, w, 64, "LIGHT", (float)light, "lx", sun_icon, COL_LIGHT, hasLight);
...
void updateRelays(bool r1, bool r2, bool r3, bool r4) {
    if (xSemaphoreTake(uiMutex, portMAX_DELAY)) {
        ...
        int w = (240 - 20) / 4; 
        drawRelayButton(4, 5, w, 34, "RL 1", r1, hasRelay1);
        drawRelayButton(4 + (w+4)*1, 5, w, 34, "RL 2", r2, hasRelay2);
        drawRelayButton(4 + (w+4)*2, 5, w, 34, "RL 3", r3, hasRelay3);
        drawRelayButton(4 + (w+4)*3, 5, w, 34, "RL 4", r4, hasRelay4);
...
void scrollFooter() {
    if (xSemaphoreTake(uiMutex, portMAX_DELAY)) {
        ...
        for (int i=0; i<20; i++) {
            uint16_t color = sprFooter.color565(0, i*1.5, i*2);
